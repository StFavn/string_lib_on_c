...
#define actual(str, n) s21_strchr(str, n)
#define expected(str, n) strchr(str, n)
...
START_TEST(c_is_null_symb_all_vals) {
  const char *str = all_vals[_i];
  const char c = '\0';
...
Suite *suite_s21_strchr() {
  Suite *suite = suite_create("suite_s21_strchr");
  ...
  tcase_add_test(anomaly_cases, no_c_in_anomaly_vals);
  tcase_add_loop_test(anomaly_cases, c_not_char_all_vals, 0, anomaly_elems);
  tcase_add_loop_test(anomaly_cases, c_big_int_all_vals, 0, all_elems);
  tcase_add_loop_test(anomaly_cases, c_is_null_symb_all_vals, 0, all_elems);
